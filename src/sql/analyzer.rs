...
use crate::sql::parser::{Statement, Expression, BinaryOperator, UnaryOperator};
use crate::types::{DataType, Value, Schema, ColumnDefinition};
...
pub enum SemanticError {
    ...
    InsertColumnMismatch { expected: usize, actual: usize },
...
impl<'a> SemanticAnalyzer<'a> {
    ...
    pub fn analyze(&self, stmt: Statement) -> Result<AnalyzedStatement, SemanticError> {
        ...
        match &stmt {
            ...
            Statement::Select { from_clause, where_clause, select_list, .. } => {
                self.analyze_select(from_clause, where_clause, select_list, 
                                  &mut table_schemas, &mut expression_types)?;
            }
            Statement::Insert { table_name, columns, values } => {
                self.analyze_insert(table_name, columns, values, 
                                  &mut table_schemas, &mut expression_types)?;
            }
            Statement::Update { table_name, assignments, where_clause } => {
                self.analyze_update(table_name, assignments, where_clause, 
                                  &mut table_schemas, &mut expression_types)?;
            }
            Statement::Delete { table_name, where_clause } => {
                self.analyze_delete(table_name, where_clause, 
                                  &mut table_schemas, &mut expression_types)?;
            }
        }
    ...
    fn analyze_insert(
        ...
        for value_row in values {
            ...
            for (value_expr, target_column) in value_row.iter().zip(target_columns.iter()) {
                let value_type = self.analyze_expression(value_expr, table_schemas, expression_types)?;
    ...
    fn analyze_update(
        ...
        for assignment in assignments {
            ...
            let value_type = self.analyze_expression(&assignment.value, table_schemas, expression_types)?;
    ...
    fn analyze_expression(
        ...
        let expr_type = match expr {
            ...
            Expression::UnaryOp { op, expr: operand } => {
                let operand_type = self.analyze_expression(operand, table_schemas, expression_types)?;
            ...
            Expression::In { expr: operand, list } => {
                ...
                for item in list {
                    let item_type = self.analyze_expression(item, table_schemas, expression_types)?;
        ...
        match op {
            ...
            Add | Subtract | Multiply | Divide | Modulo => {
                if self.is_numeric_type(left_type) && self.is_numeric_type(right_type) {
                    ...
                    if matches!(left_type, DataType::Double) || matches!(right_type, DataType::Double) {
                        Ok(DataType::Double)
                    } else if matches!(left_type, DataType::Float) || matches!(right_type, DataType::Float) {
                        Ok(DataType::Float)
                    } else if matches!(left_type, DataType::BigInt) || matches!(right_type, DataType::BigInt) {
                        Ok(DataType::BigInt)
                    } else {
                        Ok(DataType::Integer)
                    }
...
mod tests {
    use super::*;
    use crate::sql::parser::{Statement, Expression, BinaryOperator};
    ...
    fn test_analyze_create_table() {
        ...
        let stmt = parse_sql("CREATE TABLE test (id INT PRIMARY KEY, name VARCHAR NOT NULL)").unwrap();
    ...
    fn test_analyze_duplicate_table() {
        ...
        assert!(matches!(result, Err(SemanticError::TableAlreadyExists { .. })));
    }
    ...
    fn test_analyze_insert_column_mismatch() {
        ...
        let stmt = parse_sql("INSERT INTO users (name, age) VALUES ('Alice', 25, 'extra')").unwrap();
        let result = analyzer.analyze(stmt);
        
        assert!(matches!(result, Err(SemanticError::InsertColumnMismatch { .. })));
    }
    ...
    fn test_analyze_binary_operations() {
        ...
        match result {
            Ok(_) => {}
            Err(e) => panic!("Expected success for arithmetic operation, got error: {:?}", e),
        }
